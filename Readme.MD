...
> **동일한 양자 회로에 대해**  
> - 노이즈 없는 이론적 기대값과  
> - 노이즈가 포함된 측정 결과를 동시에 생성하고,  
> 이를 이용해 **회귀 모델이 직접 이상적 관측값을 복원하도록 학습합니다.
...
```mermaid
flowchart LR
    A[랜덤 2-qubit 회로 생성] --> B[Noiseless 시뮬레이션<br/>Statevector]
    B --> C[이상적 ⟨Z ⊗ Z⟩<br/>Ground Truth]

    A --> D[측정 연산 추가]
    D --> E[노이즈 모델 샘플링]
    E --> F[Qiskit Aer<br/>Noisy Simulation]
    F --> G[Measurement Counts]

    G --> H[Feature Engineering]
    C --> I[Supervised Learning]
    H --> I
    I --> J[회귀 기반 ML-QEM]
    J --> K[Mitigated ⟨Z ⊗ Z⟩]
```
