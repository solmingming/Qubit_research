...
### **Graph-aware Quantum Feature Selection for Deepfake Detection**

<p>
  <em>
    고차원 딥페이크 특징을 그래프로 이해하고,<br/>
...
<img src="https://img.shields.io/badge/Deepfake%20Detection-High--Dimensional%20Features-111111?style=for-the-badge" alt="Deepfake Detection"/>
<img src="https://img.shields.io/badge/M--FIG-Feature%20Interaction%20Graph-2C5BB4?style=for-the-badge" alt="M-FIG"/>
...
중복성, 연산 비용 증가, 해석 가능성 저하라는 문제를 함께 가진다.
...
여러 가능성을 동시에 탐색하는 **중첩(Superposition)** 개념에서 영감을 받았다.

본 프로젝트는 딥페이크 이미지 탐지를 위한  
**고차원 feature selection 문제**를 양자 최적화 관점에서 해결하고자 한다.

이를 위해 512차원 딥페이크 feature를  
**M-FIG(Multi-Feature Interaction Graph)**로 모델링하고,  
feature 간 관계를 반영한 그래프 기반 클러스터링을 통해  
QAOA가 처리 가능한 작은 block 단위로 분할한다.

각 block에서는 **QUBO 정식화와 Block-QAOA 최적화**를 수행하여,  
탐지 성능에 기여하는 핵심 feature subset을 선택한다.

본 연구는 제한된 NISQ 환경에서도 적용 가능한  
**graph-aware quantum feature selection pipeline**을 구현하는 것을 목표로 한다.
---
...
회귀 모델이 **측정 오류를 직접 보정하도록 학습**한다.

본 연구는 *Adaptive Neural Network for Quantum Error Mitigation* 논문의 아이디어를 참고하여  
**측정 통계 + 회로 구조 정보를 함께 사용하는 회귀 기반 QEM 파이프라인**을 구현한다.
